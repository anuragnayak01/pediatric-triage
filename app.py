...
st.markdown(
    ...
    .sev-title { font-size: 1.35rem; font-weight: 700; margin-bottom: 0.3rem; font-family: 'Plus Jakarta Sans', sans-serif; }
    ...
    .action-normal {
        ...
        padding: 0.85rem 1.1rem; font-size: 0.95rem; color: #065f46;
        font-weight: 500; margin-top: 0.5rem;
    }
    .action-urgent {
        background: #fff1f2; border: 1px solid #fca5a5; border-radius: 10px;
        padding: 0.85rem 1.1rem; font-size: 0.95rem; color: #991b1b;
    ...
    .disc-box {
        ...
        padding: 0.85rem 1rem; font-size: 0.85rem; color: #78350f;
        line-height: 1.65; margin-top: 0.4rem;
    }

    .badges { display: flex; gap: 0.5rem; flex-wrap: wrap; margin: 0.6rem 0 1.6rem; }
    .badge {
        background: #e0f2fe; color: #0369a1; border-radius: 999px;
        padding: 0.25rem 0.8rem; font-size: 0.76rem; font-weight: 600;
    }
    ...
    with st.expander("📚 Medical Evidence Reviewed"):
        if output.retrieved_evidence:
            for i, ev in enumerate(output.retrieved_evidence, 1):
                link = f' &nbsp;·&nbsp; <a href="{ev.source_url}" target="_blank" style="color:#0d9488;font-size:0.76rem;">View source ↗</a>' if ev.source_url else ""
                st.markdown(
                    f"""<div class="ev-item">
...
def main():
    with st.sidebar:
        ...
        st.divider()
        st.caption("⚠️ Not a diagnostic tool. Always consult a qualified pediatrician.")
    ...
    with st.form("triage_form"):
        ...
        with col2:
            temperature = st.number_input("Temperature (optional)", min_value=95.0, max_value=107.0, value=None)
        ...
        st.write("")
        submitted = st.form_submit_button("🔍  Assess Urgency", use_container_width=True, type="primary")
